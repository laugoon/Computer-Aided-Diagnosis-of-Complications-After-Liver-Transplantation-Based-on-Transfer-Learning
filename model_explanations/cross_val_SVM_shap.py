...
def train_1label_SVM_cross_val_shap(X,all_lable,label_idx):
    print('------------{}--------------'.format(y_columns[label_idx]))
    # train_data_lable = all_train_lable[y_columns[label_idx]]
    # test_data_lable = all_test_lable[y_columns[label_idx]]
    ...
    recall=0
    f1=0
    # shap_values_all=np.empty(shape=(0,50))
    shap_values_all=None
    global_shap_values_1fold=np.empty(shape=(n_splits,50))
    fold_count=0
    ...
    for train_index, test_index in skfolds.split(X, Y):
        ...
        # KernelExplainer
        # 1.创建解释器对象
        explainer = shap.KernelExplainer(clone_clf.predict_proba, X_train_folds, link="logit")
        # 2.计算各个样本的局部shapley值
...
def TCA_SVM_cross_val_shap_train(label_num=9):
    ...
    for i in range(label_num):
        ...
        recall[i], f1[i], global_shap[i]=train_1label_SVM_cross_val_shap(data,label,i)	
    # 保存预测后结果
    recall = pd.DataFrame(recall)
    f1 = pd.DataFrame(f1)
    global_shap = pd.DataFrame(global_shap)
    recall.to_csv('./data/{}TCA+SVM_cross_val_recall.csv'.format(curtime), index=False)
    f1.to_csv('./data/{}TCA+SVM_cross_val_f1.csv'.format(curtime), index=False)
    global_shap.to_csv('./data/{}TCA+SVM_cross_val_shap.csv'.format(curtime), index=False)
...
def PCA_SVM_cross_val_shap_train(label_num=9):
    ...
    f1 = pd.DataFrame(f1)
    global_shap = pd.DataFrame(global_shap)
    recall.to_csv('./data/{}PCA+SVM_cross_val_recall.csv'.format(curtime), index=False)
    f1.to_csv('./data/{}PCA+SVM_cross_val_f1.csv'.format(curtime), index=False)
    global_shap.to_csv('./data/{}PCA+SVM_cross_val_shap.csv'.format(curtime), index=False)
...
if __name__ == '__main__':
    ...
    # 修改源域目标域特征+新目标TCA+SVM+交叉验证+SHAP
