...
''''上采样+无shap'''
def train_1label_TCA_SVM_ROS_tune(Xs,all_ys,Xt,all_yt,label_idx):
    print('------------{}--------------'.format(y_columns[label_idx]))
    features_num=Xs.shape[1]
    # 查看形状
    print("shape of X:{}\nshape of Y:{}".format(Xs.shape,all_ys.shape))
    ...
    # #训练模型
    # svm_model = SVC(kernel='rbf', C=100.0,gamma=100.0, random_state=0, probability=True)
    '''单次分割做参数选择'''
    Xt_train_folds, Xt_test_fold, yt_train_folds, yt_test_fold = ms.train_test_split(Xt, Yt, test_size=0.33, random_state=7)
    n_components=50
    ...
    params = [
            ...
            {'kernel': ['rbf'], 'C': [1, 10, 20, 100], 'gamma':[1, 0.1, 0.01, 0.001]}
            ]
    
    svm_model = ms.GridSearchCV(SVC(probability=True), 
                            params, 
                            refit=True,
    ...
    # 预测
    # 选取最优参数的模型来测试模型
    model_best = svm_model.best_estimator_
    y_pred = model_best.predict(Xt_test_new)
...
# 上采样无shap
def TCA_SVM_cross_val_ROS_tune_train(label_num=9):
    # train_data = pd.read_csv('./data/TCA_source_data.csv')
    # test_data = pd.read_csv('./data/TCA_target_data.csv')
    # source_data_lable = pd.read_csv('./data/fill_labels.csv')
    # target_data_lable = source_data_lable.copy(deep=True)
    ...
    # for i in range(5):
        # 无shap+有precision
        # recall[i], f1[i], prc[i]=train_1label_TCA_SVM_ROS_unshap(xs,ys, xt,yt,i)
        recall[i], f1[i], prc[i]=train_1label_TCA_SVM_ROS_tune(xs,ys, xt,yt,i)
        # return_values=train_1label_TCA_SVM_ROS_unshap(xs,ys, xt,yt,i)
        # print("return_values:",return_values)
        # # shap+有precision
        # recall[i], f1[i], prc[i], global_shap[i]=train_1label_TCA_SVM_ROS_shap(xs,ys, xt,yt,i)
    ...
    # 设置表头
    performance.columns = ['F1','Recall','Precision']
    performance.to_csv('./data/imbalance_experiment/{}TCA+SVM_ROS_tune_performance.csv'.format(curtime), index=False)   
    # global_shap.to_csv('./data/wrapper_experiment/{}TCA+SVM_wrapper_shap.csv'.format(curtime), index=False)
    print('TCA Performance saved')
...
if __name__ == '__main__':
    ...
    TCA_SVM_cross_val_ROS_tune_train(9)#训练集上采样无shap
