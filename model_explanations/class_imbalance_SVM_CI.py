...
def train_1label_TCA_SVM_ROS_unshap(Xs,all_ys,Xt,all_yt,label_idx):
    ...
    # xt_train, xt_test, yt_train, yt_test 
        ...
        #     yt_test_fold, y_pred, score_fun=f1_score, seed=0
        # )
        # print("F1={:.2f}, 95% CI: {:.2f}-{:.2f}".format(score, ci_lower, ci_upper))

        # CI_f1[fold_count][0]=score
        # CI_f1[fold_count][1]=ci_lower
        # CI_f1[fold_count][2]=ci_upper
        # ACC
        score, ci_lower, ci_upper, scores = stat_util.score_ci(
            yt_test_fold, y_pred, score_fun=accuracy_score, seed=0
        )
        CI_ACC[fold_count][0]=accuracy_score(yt_test_fold, y_pred)
        CI_ACC[fold_count][1]=ci_lower
        CI_ACC[fold_count][2]=ci_upper
        print("ACC={:.2f}/{:.2f}, 95% CI: {:.2f}-{:.2f}".format(score, CI_ACC[fold_count][0], ci_lower, ci_upper))
        ...
        # plt.title('Histogram of {} Accuracy'.format(y_columns[label_idx]))  # 设置标题
        # plt.xlabel('Accuracy')
        # plt.ylabel('Frequency')
        # # plt.legend(loc='upper left')
        # plt.savefig("./fig/CI/{} ACC_CI{}.png".format(y_columns[label_idx],fold_count))
        # ##################ROC#########################
        ...
        #################保存模型################
        #保存模型
        model_file = './save_model/{}_{}_TCA+SVM{}.model'.format(curtime,y_columns[label_idx],fold_count)
    ...
    # 转换格式, 设置表头
    # # F1
    # CI_f1 = pd.DataFrame(CI_f1)
    # CI_f1.columns = ['F1','ci_lower','ci_upper']
    # CI_f1.to_csv('./data/CI/{}_{}_F1_TCA+SVM_ROS_CI.csv'.format(curtime,y_columns[label_idx]), index=False) 
    # ACC
    CI_ACC = pd.DataFrame(CI_ACC)
    CI_ACC.columns = ['ACC','ci_lower','ci_upper']
    CI_ACC.to_csv('./data/CI/{}_{}_ACC_TCA+SVM_ROS_CI.csv'.format(curtime,y_columns[label_idx]), index=False) 
...
# 上采样无shap
def TCA_SVM_cross_val_ROS_CI_train(label_num=9):
    # xs = pd.read_csv('./data/TCA_source_data.csv')
    # xt = pd.read_csv('./data/TCA_target_data.csv')
...
if __name__ == '__main__':
    ...
    TCA_SVM_cross_val_ROS_CI_train(9)#训练集上采样无shap
