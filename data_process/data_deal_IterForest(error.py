...
# 随机森林以及基于随机森林的填充都不能基于唯一特征'序列号', 需要先把序列号删除
 train_data.drop( ['序列号','RH(阳1阴0)','血小板POD12'],axis = 1,inplace=True)
...
# 分类特征处理begin
# # 因为分类特征的正常值都在0.1.2范围内, 没有负的, 所以先将缺失值填充为-1,转为int类型后再将所有-1替换为NA
#  categorical_columns=['O型','A型','B型','AB型','乙肝携带','术式(经典1背驮0)','VII因子','多巴胺出室','再灌注后综合征','切脾','肝肾联合移植']
#  continuous_columns=['热缺血时间min','冷缺血时间min','红细胞','血浆','自体血','4%白蛋白','2%白蛋白','纯白蛋白g','万汶','佳乐施','胸水','速尿mg','甘露醇ml','凝血酶原复合物U','红细胞POD0','红细胞POD1','红细胞POD2','红细胞POD3','红细胞POD4','红细胞POD5','红细胞POD6','红细胞POD7','红细胞POD8','红细胞POD9','红细胞POD10','红细胞POD11','红细胞POD12','红细胞POD13','红细胞POD14','红细胞POD14+','血浆POD0','血浆POD1','血浆POD2','血浆POD3','血浆POD4','血浆POD5','血浆POD6','血浆POD7','血浆POD8','血浆POD9','血浆POD10','血浆POD11','血浆POD12','血浆POD13','血浆POD14','血浆POD14+','血小板POD0','血小板POD1','血小板POD2','血小板POD3','血小板POD4','血小板POD5','血小板POD6','血小板POD7','血小板POD8','血小板POD9','血小板POD10','血小板POD11','血小板POD13','血小板POD14','血小板POD14+']
#  all_data.fillna({'O型':'-1','A型':'-1','B型':'-1','AB型':'-1','乙肝携带':'-1','术式(经典1背驮0)':'-1','VII因子':'-1','多巴胺出室':'-1','再灌注后综合征':'-1','切脾':'-1','肝肾联合移植':'-1'}, inplace = True)
# #  all_data[category_columns].replace(to_replace = np.nan,value =-1,inplace=True)#不行
# #  看填充是否成功
#  print('all_data info:\n',all_data[categorical_columns].info(verbose=True, null_counts=True))
#  all_data[categorical_columns]= all_data[categorical_columns].astype('int')
# #  all_data['乙肝携带']= all_data['乙肝携带'].apply(pd.to_numeric, downcast="integer")
#  #  看类型转换是否成功
#  print('all_data.dtypes:\n',all_data['O型'].dtypes,all_data['A型'].dtypes,all_data['B型'].dtypes,all_data['AB型'].dtypes,all_data['乙肝携带'].dtypes,all_data['术式(经典1背驮0)'].dtypes,all_data['VII因子'].dtypes,all_data['多巴胺出室'].dtypes,all_data['再灌注后综合征'].dtypes,all_data['切脾'].dtypes,all_data['肝肾联合移植'].dtypes)#查看DF内部数据类型
# #  all_data.loc[all_data[category_columns] == -1, category_columns] = pd.NA
#  for col in categorical_columns:
#   #  all_data[col].replace(to_replace = -1,value =pd.NA,inplace=True)
#    all_data[col].replace(to_replace = -1,value =np.nan,inplace=True)
# #  all_data[category_columns].replace(to_replace = -1,value =np.nan,inplace=True)
# #  看逆填充是否成功
#  print('all_data info:\n',all_data[categorical_columns].info(verbose=True, null_counts=True))
# 分类特征处理end

# #  处理回归特征, 使之满足缺失值填充时回归的判断条件(已调整到前面先执行)
#  all_data[continuous_columns]=all_data[continuous_columns]+3.14e-10
# #  看值调整是否成功
#  print('continuous_columns:\n',all_data[continuous_columns])

 # 先转成ndarray输入Miss forest再转回dataframe
#  train_data = pd.DataFrame(iterforest.MissForest().complete(all_data.values))#原执行代码
 #  分开调试
 myMissForest=iterforest.MissForest()
 fillNA_array=None
 try:
   fillNA_array=myMissForest.complete(all_data.values)
 except ValueError as Argument:
  print('ValueError:',Argument)
  print('categorical_index(except):\n{}'.format(myMissForest.coltype_dict))  
  # print('X_values:\n',myMissForest.?)
 else:
  print("fillNA成功")
 train_data = pd.DataFrame(fillNA_array)
...
#这个暂时不能删, 因为之后PH-150两个都用到了
 ...
 medical_data_fill_na=medical_data
 #原代码变量保存end
